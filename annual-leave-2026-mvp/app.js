...
function parseWorkRecords(rows) {
  ...
  for (const r of rows) {
    ...
    const cls = classifyRecord(kind, reason);
...
function renderDetails(allDetails) {
  // 개인별: 종별 취합표 + 검토필요 목록 + (선택) 전체 목록
  const html = allDetails.map(d => {
    ...
    const allRows = showRecs.map(r => `
      <tr>
        ...
        <td>${r.reason||''}</td>
        <td>${r.approval||''}</td>
        <td>${r.cls.category}</td>
        <td>${r.cls.vacationCredit ? 'Y' : 'N'}</td>
      </tr>
    `).join('');

    const allTable = allRows
      ? `<table>
          <thead><tr><th>기간</th><th>종별</th><th>일수/기간</th><th>사유</th><th>결재</th><th>분류</th><th>방학크레딧</th></tr></thead>
          <tbody>${allRows}</tbody>
        </table>`
    ...
    return `
      <details>
        <summary><b>${d.name}</b> (${d.personalNo||'개인번호 없음'}) - 총 ${d.recordCount}건 / 검토필요 ${d.reviewCount}건</summary>
        ...
        <details>
          <summary>전체 복무 목록 보기 (UI 표시 ${allRecLimit}건 제한${truncated ? ', 나머지는 엑셀 다운로드 확인' : ''})</summary>
          ${allTable}
        </details>
      </details>
    `;
...
async function run() {
  ...
  try {
    ...
    const personMap = new Map(persons.map(p => [p.key, p]));
    ...
    LAST_RESULT = { calendar, rows: resultRows, allRecords, detailRows };
